                    ...
                    <div class="card-body">
                        <?php include 'konek.php'; ?>
                              <form method="post" >
                                  ...
                                  <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="idmargin_penjualan" id="idmargin_penjualan" class="form-control" required>
                                            <?php
                                            $queryMargin = $db->query("SELECT * FROM margin_penjualan");
                                            while ($margin = $queryMargin->fetch_assoc()) {
                                                echo "<option value='" . $margin['idmargin_penjualan'] . "'>" . $margin['persen'] ." % ". "</option>";
                                            }
                                            ?>
                                        </select>
                                    </div>
                                ...
                                <button type="submit" name="tambah" class="btn btn-info">Tambah Penjualan</button>
                              </form>
                        ...
                        <?php
                         if (isset($_POST['tambah'])) {
                            ...
                            $queryInsert = $db->query("INSERT INTO penjualan (created_at, iduser, idmargin_penjualan)
                                                        VALUES ( NOW(),'$iduser', '$idmargin_penjualan')");

                            if ($queryInsert) {

                                $idpenjualan = $db->insert_id;

                                echo "<script>window.location = 'add2_penjualan.php?idpenjualan=$idpenjualan';</script>";
                            } else {
              ...
              <div class="card-body">
                <table id="example2" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    include 'konek.php';
                    $query = $db->query("SELECT * FROM detail_penjualan");
                    foreach ($query as $tampil) {
                    ?>
                  <tr>
                    <td><?php echo $tampil["iddetail_penjualan"];?></td>
                    <td><?php echo number_format($tampil['harga_satuan'], 0, ',', '.'); ?></td>
                    <td><?php echo $tampil["jumlah"];?></td>
                    <td><?php echo number_format($tampil['subtotal'], 0, ',', '.'); ?></td>
                    <td><?php echo $tampil["idpenjualan"];?></td>
